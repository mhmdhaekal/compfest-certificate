...
export default class CertificateGeneratorService {
	async generateCertificate({
		...
		return new Promise(async (resolve, reject) => {
			try {
				...
				doc.fillColor("white");
				let centerBetweenLabelAndBottom =
					labelPosition + (bottomTextPosition - labelPosition) / 2;
	...
	private async generateQrCode({
		certificateId,
	}: { certificateId: string }): Promise<Buffer> {
	...
	private async getFont(): Promise<ArrayBuffer> {
		let fontResponse = await fetch(
			`https://asset.compfest.id/font/Lexend-Bold.ttf`,
		);
